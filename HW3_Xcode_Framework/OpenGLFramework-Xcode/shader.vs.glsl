...
void main() 
{
    ...
    vertexInView = gl_Position;
    vec4 P = um4v * um4m * vec4(aPos, 1.0);
    N = mat3(um4v * um4m) * aNormal;
    L = (cur_light_mode == 0) ? light.position: light.position - aPos;
    V = -P.xyz;
    vertex_color = computeLight(N, L, V, cur_light_mode);
}
